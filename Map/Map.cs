...
namespace Map
{
    public class Map<TKey, TData> : IEnumerable<MapItem<TKey, TData>> where TKey : IComparable<TKey>, IEquatable<TKey> where TData : IComparable<TData>, IEquatable<TData>
    {
        ...
        private MapItem<TKey, TData> GetItem(TKey key) => Items.SingleOrDefault(item => item.Key.Equals(key));
        private List<MapItem<TKey, TData>> GetItems(TData data) => Items.Where(item => item.Data.Contains(data)).ToList();
